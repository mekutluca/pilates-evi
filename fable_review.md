...
## 3. Database hardening (Phase 2 — biggest correctness *and* speed wins, no app refactor needed)
...
1. **`schedule/+page.server.ts:84-93` fetches the entire `pe_appointments` table on every schedule load** (no date filter, with the per-row RLS tax on 1,264+ growing rows). Bound it to the displayed week ± the window extensions actually need, or compute conflicts via the warnings path only. *Single biggest win.*
...
1. **Drop `pe_packages.lessons_per_week`: YES.** Verified unused — the only `lessons_per_week` in app code is a *computed* view-model field in `extend/+page.server.ts:246` (`lessons_per_week: timeSlots.length`); nothing reads the DB column. Drop column + regenerate `database.types.ts`.
