...
>
> **Progress part 3 (same day):** extend flow is now transactional — `pe_extend_purchase` plpgsql RPC creates purchases + successor links + appointments/enrollments atomically (private and group variants); both extend actions rewritten to a single `PurchaseRepository.extend()` call.
>
> **Progress part 4 (same day): §2.4 fully closed.** `pe_create_assignment` RPC (keyed unit-of-work: group lesson → teams+purchases → appointments → enrollments in one transaction) + new `SchedulingService`. All four createAssignment branches now validate first and write once — this also fixed the orphan-purchase bug where the selected-timeslots branch created purchases _before_ its enrollment guard could fail. new-assignment/+page.server.ts shrank by ~200 lines.
>
> **Remaining (next session):**
>
> - Phase 3: `AppointmentRepository`/`EnrollmentRepository` absorbing shift/cancellation utils; `NotificationService`; CRUD repositories + thin route rewrites (transfer server is still 1,100+ lines). All correctness items from the original review are now fixed.
