...
export const actions: Actions = {
    createRoom: async ({ request, locals: { supabase, user, userRole } }) => {
        ...
        if (createError) {
            return fail(500, { success: false, message: 'Oda oluşturulurken hata: ' + createError.message });
        }
    ...
    updateRoom: async ({ request, locals: { supabase, user, userRole } }) => {
        ...
        if (updateError) {
            return fail(500, { success: false, message: 'Oda güncellenirken hata: ' + updateError.message });
        }
