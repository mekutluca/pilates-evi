...
export const actions: Actions = {
    createTrainee: async ({ request, locals: { supabase, user, userRole } }) => {
        ...
        if (createError || !traineeData) {
            return fail(500, { success: false, message: 'Öğrenci oluşturulurken hata: ' + createError?.message });
        }
    ...
    updateTrainee: async ({ request, locals: { supabase, user, userRole } }) => {
        ...
        if (fetchCurrentError) {
            return fail(500, { success: false, message: 'Mevcut öğrenci bilgileri alınırken hata: ' + fetchCurrentError.message });
        }
        ...
        if (updateError) {
            return fail(500, { success: false, message: 'Öğrenci güncellenirken hata: ' + updateError.message });
        }

        // Update bidirectional relationships
        // Find added and removed relationships
        const addedIds = selectedTraineeIds.filter(id => !currentRelatedIds.includes(id));
        const removedIds = currentRelatedIds.filter(id => !selectedTraineeIds.includes(id));
        ...
        if (removedIds.length > 0) {
            ...
            if (!fetchRemovedError && removedTrainees) {
                const removeUpdates = removedTrainees.map(async (trainee) => {
                    const currentIds = trainee.related_trainee_ids || [];
                    const newIds = currentIds.filter(id => id !== traineeId);
    ...
    deleteTrainee: async ({ request, locals: { supabase, user, userRole } }) => {
        ...
        if (fetchAllError) {
            return fail(500, { success: false, message: 'İlgili öğrenci bilgileri alınırken hata: ' + fetchAllError.message });
        }

        if (allTrainees) {
            // Find trainees that have the deleted trainee in their relationships
            const traineesToUpdate = allTrainees.filter(trainee => 
                trainee.related_trainee_ids && trainee.related_trainee_ids.includes(traineeId)
            );

            // Remove the deleted trainee's ID from their relationships
            const cleanupUpdates = traineesToUpdate.map(async (trainee) => {
                const newIds = trainee.related_trainee_ids.filter(id => id !== traineeId);
        ...
        if (deleteError) {
            return fail(500, { success: false, message: 'Öğrenci silinirken hata: ' + deleteError.message });
        }
